...
--
-- Host: 192.168.1.6    Database: Auditing-Linux-Minimal
-- ------------------------------------------------------
...
CREATE TABLE `Command_Types` (
  `idCommand_Type` int NOT NULL,
  `Command_Type_Description` varchar(45) NOT NULL,
  PRIMARY KEY (`idCommand_Type`)
...
INSERT INTO `Command_Types` VALUES (1,'A'),(2,'M');
...
CREATE TABLE `Findings_Commands` (
  ...
  `idCommand_Type` int DEFAULT '1',
  PRIMARY KEY (`idFinding`,`idFinding_Command`),
  KEY `fkCommandType_idx` (`idCommand_Type`),
  CONSTRAINT `fkCommandType` FOREIGN KEY (`idCommand_Type`) REFERENCES `Command_Types` (`idCommand_Type`) ON UPDATE CASCADE,
  CONSTRAINT `fkIdFinding` FOREIGN KEY (`idFinding`) REFERENCES `Findings` (`idFinding`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `Findings_Commands` VALUES ('V-204452',1,'grep -i clean_requirements_on_remove /etc/yum.conf','N',1),('V-204461',1,'pwck -r','N',1),('V-204486',1,'cat /etc/fstab | grep /dev/shm','N',1),('V-204486',2,'mount | grep /dev/shm','N',1),('V-204493',1,'grep /home /etc/fstab','N',1),('V-204494',1,'grep /var /etc/fstab','N',1),('V-204495',1,'grep /var/log/audit /etc/fstab','N',1),('V-204495',2,'mount | grep \"/var/log/audit\"','N',1),('V-204496',1,'systemctl is-enabled tmp.mount','N',1),('V-204496',2,'grep -i /tmp /etc/fstab','N',1),('V-204498',1,'yum list installed aide','N',1),('V-204498',2,'find / -name aide.conf','Y',1),('V-204499',1,'yum list installed aide','N',1),('V-204499',2,'find / -name aide.conf','Y',1),('V-204576',1,'grep \"maxlogins\" /etc/security/limits.conf /etc/security/limits.d/*.conf','N',1),('V-204605',1,'grep pam_lastlog /etc/pam.d/postlogin','N',1),('V-204608',1,'grep hosts /etc/nsswitch.conf','N',1),('V-204608',2,'ls -al /etc/resolv.conf','N',1),('V-204608',3,'grep nameserver /etc/resolv.conf','N',1),('V-204608',4,'lsattr /etc/resolv.conf','Y',1);
...
-- Dump completed on 2021-12-26 14:33:09
